...
From lecture we know that Analysis of Variance is designed to provide a single test of a null hypothesis of equal group means with a desired signiﬁcance level. It is a generalization of the equal-variance t-test to the case where the number of means to be compared is greater than 2. We are testing for difference in means of various groups, hense using ANOVA would be very convenient and efficient.
...
After some exploratoy analysis of the data we found that the variances showed some difference, hense we might not be able to satify assumptions for ANOVA for each group. This lead us to conduct a Welch T-Test as well, addressing the difference in variance, addressing the issues that might arise from ANOVA.

**Assumptions:**
For the each of the test we are working under the assumption that the samples included in the Kaggle Survey dataset are independent, meaning that no answers provided by someone that took the survey affected a different entry. Moreover, we are working under the assumption that we have large enought data for each group to account for normality in both the ANOVA and Welch t-test. For the ANOVA and Regression models, we are also assuming equality in the variance of the groups. Finally, we persume there to be normality and liniarity for the models to perform either Linear or a Generalize linear model.
...
The boxplots for both the Job Title groups and Language preference groups, seem to have minimal difference in their variance from what one can tell from the charts. The density plots for each group, also shows a general normality to the distribution of the groups. Given these, we proceeded with the assumption of equal variance and normality needed for ANOVA and Regression.

For our tests, we ran under the hypothesis:
$$
...
From the ANOVA tests, when testing at the $$\alpha=0.05$$ level, we got found for the Jobs Title Groups to have a p-value = `r formatC(anova.gen.jobs[[1]][["Pr(>F)"]][1],format = 'e')`, meaning we have strong evidence to reject the null hypothesis and that the mean salary of the Job Title groups are not equal. On the other hand, for the Language groups we got the p-value = `r formatC(anova.gen.lang[[1]][["Pr(>F)"]][1],format = 'e')`, meaning we do not have strong evidence to reject the null hypothesis or equal mean salary among Language Preference.
